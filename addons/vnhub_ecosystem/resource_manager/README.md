...
- По запросу качает пак (на вебе — в `user://`, с кэшем в IndexedDB), монтирует его и предзагружает тяжёлые ассеты через `ResourceLoader.load_threaded_request` (не морозит UI, если включены потоки).
...
2. Заполни в инспекторе:
   - `base_url` — база скачивания (заканчивается на `/`);
...
И этот `level_2.pck` должен быть доступен по `base_url + "level_2.pck"`.
...
- `0 .. pack_weight` — скачивание пака (на вебе реальный прогресс по байтам; на десктопе и при кэш-попадании — сразу `pack_weight`).
...
- [ ] Задан `base_url` (в конфиге или `ResourceManager.base_url`)
- [ ] Паки собраны (`--export-pack`) и доступны по `base_url`
